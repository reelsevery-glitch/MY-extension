/**
 * Myhome.ge-დან განცხადების მონაცემების დამუშავება
 * next_data პირდაპირ Extension-იდან მოდის (უკვე parse-ებული)
 */
async function scrape_myhome(url, next_data, owner_phone) {
  try {
    const data = typeof next_data === 'string' ? JSON.parse(next_data) : next_data;
    
    // myhome იყენებს react-query dehydrated state
    const queries = data?.props?.pageProps?.dehydratedState?.queries || [];
    
    // ძირითადი განცხადების მონაცემები
    let statement = null;
    ...
    if (!statement) throw new Error('განცხადების მონაცემები ვერ მოიძებნა next_data-ში');
    ...
    // Myhome API (api-statements.tnet.ge) -სთვის შაბლონი
    const template = {
      ...
      currency_id: statement.currency_id || 1, // 1=USD, 2=GEL, 3=EUR
